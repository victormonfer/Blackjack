...
void iniciBaraja(int(&arrayBaraja)[4][13], int(&arrayCarta)[4][13]) {//inicializar la baraja (I) los palos (J) nuemros
	for (int i = 0; i < 4; i++) {        //los palos de 0 a 3 que son cuatro palaos diamantes trevoles picas y corazones
		for (int j = 0; j < 13; j++) {  //los numeros que hay de 0 a 13


			arrayBaraja[i][j] = 1;     //para poner las cartas emìezan con 1 que significa que epuede salir si esta 0 no puede dartela otra vez

			if (j > 0 && j < 10) {     
				arrayCarta[i][j] = j + 1;// 
			}
			else if (j >= 10 && j < 14) {
				arrayCarta[i][j] = 10;        // las cartas J Q K que valgan 10
			}
